...
if((!isset($_SESSION['userId']) && empty($_SESSION['userId'])) && (!isset($_SESSION['userName']) && empty($_SESSION['userName']))) {
    ...
    if( isset($_POST['submit']) ){
        ...
        if($is_admin == 'yes'){
            ...
            if( isset($_POST['email']) && !empty($_POST['email']) ){
                ...
                $query = "SELECT * FROM instructor WHERE mail='$validate_email' ";
                $result = mysqli_query($connection, $query);
                
                if(mysqli_num_rows($result) > 0){
                    $email_err_msg = '<b class="text-danger text-center">Email already exists try with different.</b>';
                }
            }
            ...
            if( ( isset($name) && !empty($name) ) && ( isset($validate_email) && !empty($validate_email) ) && ( isset($newfilename) && !empty($newfilename) ) && ( isset($phone) && !empty($phone) ) && ( isset($description) && !empty($description) ) && ( isset($qualification) && !empty($qualification) )){
                ...
                $insert_query = "INSERT INTO instructor(name, mail, phone , image, qualification, description) VALUES ('$name','$validate_email','$phone','$newfilename','$qualification','$description')";
             ...
             $message = "<div class='alert alert-danger'> 
                <p>you are not allowed to perform that operations</p><br>       
    ...
    if(isset($_GET['delete_id'])){ 
        ...
        $instructor_id = $_GET['delete_id'];

        if($is_admin == 'yes'){
      
            $message = "<div class='alert alert-danger'> 
                <p>Are you sure want to delete this Record?</p><br>
                    <form action='{$_SERVER['PHP_SELF']}?id=$instructor_id' method='post'>
...
if(isset($_POST['confirm_delete'])){
    ...
    if($result){
        header("Location: instructors.php?success=1");
    }else{
        echo "unable to delete the record! try again later.....";
    }
}
				...
				<div class="container clearfix">
                    <div class="postcontent nobottommargin">
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" id="emailid1" placeholder="Email" name="email" class="form-control custom_input" title="someone@example.com" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" required>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="phoneId1" placeholder="Phone" name="phone" class="form-control custom_input" maxlength="10" required>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <?php
                                ...
                                if(mysqli_num_rows($result) > 0){
                                    while( $row = mysqli_fetch_assoc($result) ){
                                        echo "<tr>";
                                            echo "<td>".$row["id"]."</td> <td><img src=images/instructor/".$row["image"]." style='object-fit: cover;' width='80px' height='80px'> </td> <td>".$row["name"]."</td> <td> ".$row["mail"]."</td><td>".$row["qualification"]."</td> <td>".$row["phone"]."</td>";
                                        
                                            echo '<td><a href="view.php?instructorId='.$row['id']. '" type= "button" class="btn bg_slate btn-sm">
                                            <span class="icon-eye-open"></span></a></td>';

                                            echo '<td><a href="updateinstructors.php?id='.$row['id']. '" type= "button" class="btn bg_slate btn-sm">
                                            <span class="icon-edit"></span></a></td>';
                                            
                                            echo '<td><a href="instructors.php?delete_id='.$row['id']. '" type= "button" class="btn btn-danger btn-sm">
                            ...
                            <tr>
                                <td colspan="9" id="end"><div class="text-center"><a href="instructors.php" type="button" class="btn btn-sm bg_slate"><span class="icon-plus"></span></a></div></td>
                            </tr>
                        </table>
